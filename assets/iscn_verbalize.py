...
CATEGORIES = {
    #: «Физиологическая изменчивость» — оборот клинической химии о
    #: референтных интервалах; о хромосомной находке говорят
    #: «нормальная генетическая изменчивость».
    "норма": "находки в пределах нормальной генетической изменчивости",
...
def limits(case: IF.Case, style: Style, detection_limit: Optional[float] = None,
    ...
    if detection_limit is not None:
        # предел печатается процентом в обоих регистрах: доля 0,06 читается
        # хуже, чем 6 %, а величина одна и та же
        dl = f"{_num(100 * detection_limit, 1, style.decimal_comma)} %"
        if pat:
            out.append(LIMITS_PATIENT["предел"].format(dl=dl))
        else:
            out.append(f"событие, затрагивающее менее {dl} клеток образца, "
                       "методом не выявляется; предел заявления установлен "
                       "внутренней валидацией лаборатории")
